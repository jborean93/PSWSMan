...
REALM=${AD_REALM:-PSWSMan.TEST}
PASSWORD=${AD_PASSWORD:-Password01}
NETWORK_NAME=PSWSMan-net-$( openssl rand -hex 5 )
DC_CONTAINER_ID=""
...
DC_CONTAINER_ID=$( $DOCKER_BIN run \
    --detach \
    --rm \
    --volume "$( pwd ):/tmp/PSWSMan${VOLUME_FLAGS}" \
    --env AD_REALM="${REALM^^}" \
    ...
    debian:11 /bin/bash /tmp/PSWSMan/tools/setup-samba.sh )
