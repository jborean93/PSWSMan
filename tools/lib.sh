...
lib::setup::system_requirements::el() {
    ...
    if [ x"${GSSAPI_PROVIDER}" = "xheimdal" ]; then
        ...
        # heimdal-workstation - Provides kinit for tests but not needed by PSWSMan
        dnf install -y \
            --nogpgcheck \
            --disablerepo=\*modul\* \
            heimdal-libs \
        ...
        # krb5-workstation - Provides kinit for tests but not needed by PSWSMan
        dnf install -y \
            --nogpgcheck \
            --disablerepo=\*modul\* \
            krb5-libs \
...
lib::setup::gssapi() {
    cat > /tmp/PSWSMan-krb5.conf << EOF
[libdefaults]
  rdns = false
  default_realm = ${AD_REALM^^}
EOF

    export KRB5_CONFIG="/tmp/PSWSMan-krb5.conf"
