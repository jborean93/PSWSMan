...
using unsafe AcquireCredWithPasswordFn = delegate* unmanaged[Cdecl]<
    uint*,                      // minor_status
    void*,                      // desired_name
    Helpers.gss_buffer_desc*,   // password
    uint,                       // time_req
...
{
    private readonly AcquireCredWithPasswordFn _acquireCredWithPassword =
        (AcquireCredWithPasswordFn)GetExport(module, "gss_acquire_cred_with_password");

    /// <summary>Acquires a credential handle for a principal using an explicit password.</summary>
    /// <param name="desiredName">The principal to get the credential for.</param>
    /// <param name="password">The password, which only needs to stay valid for the call.</param>
    ...
    public SafeGssapiCred AcquireCredWithPassword(
        ...
        uint* timeRec)
    {
        ...
        {
            Helpers.gss_buffer_desc passwordBuffer = new()
            {
                length = (nuint)password.Length,
                value = passwordPtr,
            };

            uint minorStatus;
            void* outputCred;
            uint majorStatus = _acquireCredWithPassword(
                &minorStatus,
                nameRef.Pointer,
                &passwordBuffer,
                timeReq,
            ...
            if (majorStatus != GSS_S_COMPLETE)
                throw new GSSAPIException(this, majorStatus, minorStatus, "gss_acquire_cred_with_password");
